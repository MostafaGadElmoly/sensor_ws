...
class PointCloudPublisher(Node):
    ...
    def __init__(self):
        super().__init__('point_filtered_cloud_publisher')
        self.subscription_top = self.create_subscription(
            String,
            '/sensor_data_top_filtered',
            self.listener_callback_top,
            10)
        self.subscription_bottom = self.create_subscription(
            String,
            '/sensor_data_bottom_filtered',
            self.listener_callback_bottom,
            10)
        self.publisher_top = self.create_publisher(PointCloud2, '/point_cloud_top_filtered', 100)
        self.publisher_bottom = self.create_publisher(PointCloud2, '/point_cloud_bottom_filtered', 100)
